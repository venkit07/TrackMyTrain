             ...
             ignoreCase:(BOOL)ignoreCase
{
    ...
    else if (textField.autocompleteType == HTAutocompleteTypeStation)
    {
        static dispatch_once_t stnOnceToken;
        static NSArray *stnAutocompleteArray;
        dispatch_once(&stnOnceToken, ^
                      {
                          stnAutocompleteArray = [StationsDataSource stationList];
                      });
        NSString *stringToLookFor;
        ...
        if (ignoreCase)
        {
            stringToLookFor = [prefixLastComponent uppercaseString];
        }
        else
        {
        ...
        for (NSString *stringFromReference in stnAutocompleteArray)
        {
            NSString *stringToCompare;
            if (ignoreCase)
            {
                stringToCompare = [stringFromReference uppercaseString];
            }
            else
            {
                stringToCompare = stringFromReference;
...
//            if ([stringToCompare hasPrefix:stringToLookFor])
                if ([stringToCompare containsString:stringToLookFor])
            {
